...
import gray from "../../images/darkBlue.jpg";
...
function ShipmentList() {
    ...
    setPreResult((prevResults) => {
      const updatedResults = prevResults.map((item) => {
        if (item.shipment_id == shipmentId) {
          item.payment = newState.toString();
        }
        return item;
      });

      console.log("Updated preResult:", updatedResults); // Log the updated preResult
  ...
  useEffect(() => {
    ...
    socket.on("recShipment", ({ state, driver_id, user_id, shipment_id }) => {
      ...
      console.log("here state_socket", state_socket);
      console.log("here shipment_id_socket", shipment_id_socket);
      handleSocketUpdate(shipment_id_socket, state_socket);
    });
  }, []);
  ...
  const BackgroundContainer = ({ children }) => (
    <div
      style={{
        ...
        backgroundSize: "auto", // Set the background size to "auto" for repeated images
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{
          height: "calc(100% - 20px)", // Adjust padding
          backgroundImage: {},
      ...
      >
        <div className="relative pt-32 pb-10 md:pt-40 md:pb-16">
          ...
          {reversedPreResult.map((item, index) => {
            ...
            return (
              <div key={index} className={classes.shipmentContainer}>
                <div className={classes.shipmentContainer2}>
                  <div className={classes.inlineContainer}>
                    <img src={plumb} className={classes.shipmentImage}></img>
                  </div>
                  <div className={classes.div1}>
                    <div className={classes.div0}>
                      <p className={classes.lineText}>Thông tin dịch vụ</p>
                    </div>
                    {/* <p>{item.senderName}</p> */}
                    {/* <p>{item.senderPhone}</p> */}
                    <p className={classes.lineText2}>
                      Dịch vụ: {item.typeOfShipment}
                    </p>
                  </div>
                  <div className={classes.div13}>
                    <div className={classes.div0}>
                      <p className={classes.lineText}>Thông tin đơn hàng</p>
                    </div>
                    {/* <p>{item.senderName}</p> */}
                    {/* <p>{item.senderPhone}</p> */}
                    <p>
                      Ngày thực hiện: {new Date(item.time).toLocaleDateString()}
                    </p>
                    <p>
                      Giờ thực hiện: {hours}:{minutes}
                    </p>
                    <p>Chi phí: {item.fee} VNĐ</p>
                  </div>
                  <div className={classes.div11}>
                    <div className={classes.div0}>
                      <p className={classes.lineText3}>Thông tin thợ</p>
                    </div>
                    <div className={classes.div4}>
                      <ul>
                        {item.driver.map((driver, driverIndex) => (
                          <li key={driverIndex}>
                            <p>Họ và tên: {driver.full_name}</p>
                            <p>Số điện thoại: {driver.phone_number}</p>
                            <p>Đánh giá: {driver.rating}</p>
...
//     // Make a POST request to your NestJS server's upload endpoint
//     axios
//       .post("https://365truck.fdssoft.com/api/upload2", formData)
//       //.post("http://localhost:3001/api/upload2", formData)
